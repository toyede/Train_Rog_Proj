...
void ATrainCharacter::Interact(const FInputActionValue& Value)
{
	...
	CollisionParams.AddIgnoredActor(this); // ?ê¸° ?ì‹ ?€ ?¸ë ˆ?´ìŠ¤?ì„œ ?œì™¸
	...
	bool bHit = GetWorld()->LineTraceSingleByChannel(
		...
		StartLocation,
		EndLocation,
		ECC_Visibility, // Visibility ì±„ë„???ˆëŠ” ?¤ë¸Œ?íŠ¸ë§?ê°ì?
		CollisionParams
	);
	...
	if (bHit)
	{
		...
		// ë¶€?ªížŒ ?¡í„°ê°€ InteractInterfaceë¥?êµ¬í˜„?ˆëŠ”ì§€ ?•ì¸
		if (HitActor && HitActor->GetClass()->ImplementsInterface(UInteractInterface::StaticClass()))
		{
            // ì²«ë²ˆì§??¸ìž(HitActor)??Interact?¨ìˆ˜ë¥??¤í–‰??ê°ì²´, this??Interact?¨ìˆ˜?ì„œ ?¸ìžë¡?ë°›ì„ ê°ì²´.
